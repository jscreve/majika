       ...
       /* File fileMetadataFolder = new File(); //Init of the folder creatin
        ...
        do {
            ...
            for(File filou: result.getFiles()) {
                if(filou.getName().equals("ups.json")) { //only execute this line if the file is named ups.json
        ...
        fileMetadata.setParents(Collections.singletonList(file.getId())); // this one got added to, this is for setting a parent to the file ups.json
